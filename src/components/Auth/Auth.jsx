    ...
    return (
        <sectinon className="auth">
            <form className="auth__form" name="auth-user" noValidate>
                ...
                <label className={`auth__label${isLoginPage ? ' auth__label_none' : ''}`} htmlFor="input-name">Имя</label>
                <input id="input-name"
                ...
                <span className="auth__text-error">Что-то пошло не так...</span>
                <button className={`auth__btn${isLoginPage ? ' auth__btn_login' : ''}`}>Зарегестрироваться</button>
                <p className="auth__text">
                    {isLoginPage ? 'Уже зарегестрированы? ' : 'Ещё не зарегестрированы? '}
                    <NavLink className="auth__link" to={isLoginPage ? '/signup' : '/signin'}>{isLoginPage ? 'Регистрация' : 'Войти'}</NavLink>
                </p>
            </form>
        </sectinon>
    )
